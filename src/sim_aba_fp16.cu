// Driver for the fp16-IA ABA experiment. Validates drift vs MuJoCo and benchmarks.
#include <cstdio>
#include <cstdlib>
#include <vector>
#include <cstring>
#include "aba_fp16.cuh"
#define CK(call) do { cudaError_t e=(call); if(e!=cudaSuccess){ \
    fprintf(stderr,"CUDA %s:%d %s\n",__FILE__,__LINE__,cudaGetErrorString(e)); exit(1);} } while(0)
#ifndef BLOCK
#define BLOCK 64
#endif

__global__ void step_kernel_fp16(float* qpos, float* qvel, int nworlds, int ksteps, float* traj, int dumpw) {
    int w = blockIdx.x*blockDim.x + threadIdx.x;
    if (w >= nworlds) return;
    float* gqp = qpos + (size_t)w*G1_NQ; float* gqv = qvel + (size_t)w*G1_NV;
    float qp[G1_NQ], qv[G1_NV];
    for (int i=0;i<G1_NQ;++i) qp[i]=gqp[i];
    for (int i=0;i<G1_NV;++i) qv[i]=gqv[i];
    float xpos[G1_NBODY*3], xquat[G1_NBODY*4], xipos[G1_NBODY*3];
    float S[G1_NV*6], qacc[G1_NV]; __half scr[ABA_SCR_H];
    __half IA[G1_NBODY*21];
    const float dt=G1_DT;
    for (int s=0;s<ksteps;++s){
        forward_kinematics(qp, xpos, xquat, xipos);
        aba_qacc_h(qp, qv, xpos, xquat, xipos, S, scr, IA, qacc);
        for (int i=0;i<G1_NV;++i) qv[i]+=dt*qacc[i];
        integrate_pos(qp, qv, dt);
        if (traj && w==dumpw) for (int i=0;i<G1_NQ;++i) traj[(s+1)*G1_NQ+i]=qp[i];
    }
    for (int i=0;i<G1_NQ;++i) gqp[i]=qp[i];
    for (int i=0;i<G1_NV;++i) gqv[i]=qv[i];
}

static void read_init(double* qp,double* qv){ FILE* f=fopen("bench/init_state.bin","rb");
    if(!f){fprintf(stderr,"run just ref\n");exit(1);} fread(qp,8,G1_NQ,f); fread(qv,8,G1_NV,f); fclose(f); }
static void init_worlds(float* dqp,float* dqv,int N,const float* q0,const float* v0){
    std::vector<float> hp((size_t)N*G1_NQ),hv((size_t)N*G1_NV);
    for(int w=0;w<N;++w){ for(int i=0;i<G1_NQ;++i)hp[(size_t)w*G1_NQ+i]=q0[i]; for(int i=0;i<G1_NV;++i)hv[(size_t)w*G1_NV+i]=v0[i]; }
    CK(cudaMemcpy(dqp,hp.data(),hp.size()*4,cudaMemcpyHostToDevice));
    CK(cudaMemcpy(dqv,hv.data(),hv.size()*4,cudaMemcpyHostToDevice)); }
static float run(float* dqp,float* dqv,int N,int nsteps,int ksteps){
    int blocks=(N+BLOCK-1)/BLOCK;
    cudaEvent_t a,b; CK(cudaEventCreate(&a)); CK(cudaEventCreate(&b)); CK(cudaEventRecord(a));
    for(int s=0;s<nsteps;s+=ksteps){ int k=min(ksteps,nsteps-s); step_kernel_fp16<<<blocks,BLOCK>>>(dqp,dqv,N,k,nullptr,-1); }
    CK(cudaEventRecord(b)); CK(cudaEventSynchronize(b));
    float ms; CK(cudaEventElapsedTime(&ms,a,b)); CK(cudaEventDestroy(a)); CK(cudaEventDestroy(b)); CK(cudaGetLastError());
    return ms; }

int main(int argc,char**argv){
    int nsteps=(argc>1)?atoi(argv[1]):300;
    double qpd[G1_NQ],qvd[G1_NV]; read_init(qpd,qvd);
    float q0[G1_NQ],v0[G1_NV]; for(int i=0;i<G1_NQ;++i)q0[i]=(float)qpd[i]; for(int i=0;i<G1_NV;++i)v0[i]=(float)qvd[i];

    if (argc>2 && !strcmp(argv[2],"validate")){   // dump world-0 trajectory (1 step/launch)
        int N=64; float *dqp,*dqv,*dtr; CK(cudaMalloc(&dqp,(size_t)N*G1_NQ*4)); CK(cudaMalloc(&dqv,(size_t)N*G1_NV*4));
        CK(cudaMalloc(&dtr,(size_t)(nsteps+1)*G1_NQ*4)); init_worlds(dqp,dqv,N,q0,v0);
        // run one big launch with traj dump
        step_kernel_fp16<<<1,N>>>(dqp,dqv,N,nsteps,dtr,0); CK(cudaDeviceSynchronize());
        std::vector<float> tr((size_t)(nsteps+1)*G1_NQ); CK(cudaMemcpy(tr.data(),dtr,tr.size()*4,cudaMemcpyDeviceToHost));
        // write q0 row too
        for(int i=0;i<G1_NQ;++i) tr[i]=q0[i];
        FILE* o=fopen("bench/aba_fp16_traj.bin","wb"); fwrite(tr.data(),4,tr.size(),o); fclose(o);
        printf("fp16 trajectory dumped (%d steps)\n",nsteps); return 0;
    }
    int Ns[]={4096,16384,65536}; int Ks[]={1,16};
    printf("fp16-IA ABA, %d steps:\n",nsteps);
    for(int N:Ns){ float *dqp,*dqv; CK(cudaMalloc(&dqp,(size_t)N*G1_NQ*4)); CK(cudaMalloc(&dqv,(size_t)N*G1_NV*4));
        for(int ks:Ks){ init_worlds(dqp,dqv,N,q0,v0); run(dqp,dqv,N,20,ks);
            init_worlds(dqp,dqv,N,q0,v0); float ms=run(dqp,dqv,N,nsteps,ks);
            printf("  N=%7d ksteps=%2d  %8.2f ms  %.3e env-steps/s\n",N,ks,ms,(double)N*nsteps/(ms/1e3)); }
        cudaFree(dqp); cudaFree(dqv); }
    return 0;
}
