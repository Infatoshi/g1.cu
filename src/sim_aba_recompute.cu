// Driver for the ABA recompute/shrink-lifetime experiment. Validates drift vs MuJoCo + benchmarks.
#include <cstdio>
#include <cstdlib>
#include <vector>
#include <cstring>
#include "aba_recompute.cuh"
...
__global__ void step_kernel_rc(float* qpos, float* qvel, int nworlds, int ksteps, float* traj, int dumpw) {
    ...
    float S[G1_NV*6], qacc[G1_NV]; __half scr[ABA_SCR_RC];
    __half IA[G1_NBODY*21];
    const float dt=G1_DT;
    for (int s=0;s<ksteps;++s){
        forward_kinematics(qp, xpos, xquat, xipos);
        aba_qacc_rc(qp, qv, xpos, xquat, xipos, S, scr, IA, qacc);
...
static float run(float* dqp,float* dqv,int N,int nsteps,int ksteps){
    ...
    for(int s=0;s<nsteps;s+=ksteps){ int k=min(ksteps,nsteps-s); step_kernel_rc<<<blocks,BLOCK>>>(dqp,dqv,N,k,nullptr,-1); }
...
int main(int argc,char**argv){
    ...
    if (argc>2 && !strcmp(argv[2],"validate")){   // dump world-0 trajectory (1 step/launch)
        ...
        step_kernel_rc<<<1,N>>>(dqp,dqv,N,nsteps,dtr,0); CK(cudaDeviceSynchronize());
        ...
        FILE* o=fopen("bench/aba_recompute_traj.bin","wb"); fwrite(tr.data(),4,tr.size(),o); fclose(o);
        printf("recompute trajectory dumped (%d steps)\n",nsteps); return 0;
    }
    int Ns[]={4096,16384,65536}; int Ks[]={1,16};
    printf("recompute ABA, %d steps:\n",nsteps);
