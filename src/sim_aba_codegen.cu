// Driver for the CODEGEN straight-line ABA experiment. Validates drift vs MuJoCo
// and benchmarks. Mirrors sim_aba_fp16.cu but calls aba_qacc_gen (no scr/IA bufs).
#include <cstdio>
#include <cstdlib>
#include <vector>
#include <cstring>
#include "aba_codegen.cuh"
...
__global__ void step_kernel_gen(float* qpos, float* qvel, int nworlds, int ksteps, float* traj, int dumpw) {
    ...
    for (int s=0;s<ksteps;++s){
        ...
        aba_qacc_gen(qp, qv, xpos, xquat, xipos, S, qacc);
...
static float run(float* dqp,float* dqv,int N,int nsteps,int ksteps){
    ...
    for(int s=0;s<nsteps;s+=ksteps){ int k=min(ksteps,nsteps-s); step_kernel_gen<<<blocks,BLOCK>>>(dqp,dqv,N,k,nullptr,-1); }
...
int main(int argc,char**argv){
    ...
    if (argc>2 && !strcmp(argv[2],"validate")){   // dump world-0 trajectory
        ...
        step_kernel_gen<<<1,N>>>(dqp,dqv,N,nsteps,dtr,0); CK(cudaDeviceSynchronize());
        ...
        FILE* o=fopen("bench/aba_codegen_traj.bin","wb"); fwrite(tr.data(),4,tr.size(),o); fclose(o);
        printf("codegen trajectory dumped (%d steps)\n",nsteps); return 0;
    }
    int Ns[]={4096,16384,65536}; int Ks[]={1,16};
    printf("codegen ABA, %d steps:\n",nsteps);
